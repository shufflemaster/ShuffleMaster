...
function pmp:OnEventBegin(vector2Value)
	self.evtMoveVector = vector2Value
end

function pmp:OnEventUpdating(vector2Value)
	self.evtMoveVector = vector2Value
end

function pmp:OnEventEnd(vector2Value)
	self.evtMoveVector = vector2Value
end

-- Game heartbeat
function pmp:OnTick(deltaTime, timePoint)
...
-- 	TransformBus.Event.SetWorldTM(self.entityId, trm)
	...
	--PhysicsComponentRequestBus.Event.SetVelocity(self.entityId, targetForward * self.Properties.movementSpeed)
	PhysicsComponentRequestBus.Event.AddImpulse(self.entityId, targetForward * self.Properties.movementSpeed*6)
end

function pmp:OnDeactivate()
